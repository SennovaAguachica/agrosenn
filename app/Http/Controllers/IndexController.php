...
class IndexController extends Controller
{
    ...
    {
        $vendedor = Vendedores::with([
            'usuario.publicaciones' => function ($query) {
                $query->where('estado', 1);
            },
            'usuario.publicaciones.productos',
            'usuario.publicaciones.precios',
            'usuario.publicaciones.unidades',
            'usuario.publicaciones.imagenes',
            'usuario.imagenesperfil'
    ...
    public function buscarProductos(Request $request)
    {
        ...
        $perfil = auth()->user();
        $asociaciones = Asociaciones::with('usuario')->get();
        $resultados = Publicaciones::with('productos.subcategoria.categorias', 'imagenes', 'usuario.vendedor.municipio.departamento', 'usuario.asociacion.municipio.departamento', 'precios', 'unidades')->join('productos as pro', 'pro.id', '=', 'publicaciones.producto_id')
            ->where('publicaciones.estado', 1)
